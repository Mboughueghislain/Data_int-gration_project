...
# Convertir le schéma en chaîne JSON
schema_json = json.dumps(schema.jsonValue())

# Parser le CSV provenant du flux avec le schéma JSON
parsed_stream = streaming_data.selectExpr(f"from_csv(value, '{schema_json}') as data").select("data.*")
